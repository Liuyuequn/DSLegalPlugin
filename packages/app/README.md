...
- **目录要你自己建好**，插件不会创建任何目录。
- **层级是硬性约束**：一个根目录之下只能有类型目录（案件文件夹不能直接放在根目录下，也不能把根目录下的
  文件夹指定成项目目录）；类型目录之下只能有项目目录，类型目录之间不能嵌套；整个插件只认一个根目录。
  位置不合法的「另行指定」在保存时会被**直接拒绝**并写明原因。
- **三级目录各自独立、可以分别指定**：此外还能在「插件设置」里
  **另行指定**若干类型目录或项目目录的**绝对路径**，它们**可以在根目录之外**（例如某个散落在别的盘、
  别的文件夹下的案件）；那种位置是**独立存在**的，不受上面三层约束。只设其中一级也能工作——根目录不再是必填项。
