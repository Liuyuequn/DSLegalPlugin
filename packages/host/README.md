...
- **`workspace.ts`** —— 扫描 `<根目录>/<类型目录>/<项目目录>/0. 协作/1. 工作日志.md`；区分完整项目与结构不完整的项目。**三级目录可各自独立设定**：类型目录与项目目录都可以另行指定绝对路径（可在根目录之外）。根目录下的子文件夹**只按结构推定**——**其下存在**符合项目目录形式规则的子文件夹才算类型目录（**名字完全不参与判定**），否则完全不扫。
