...
- **`config.ts`** —— 插件配置（schemastery）。**没有**"类型目录名 → 服务类别"的映射表。
- **`workspace.ts`** —— 扫描 `<根目录>/<类型目录>/<项目目录>/0. 协作/1. 工作日志.md`；区分完整项目与结构不完整的项目。**三级目录可各自独立设定**：类型目录与项目目录都可以另行指定绝对路径（可在根目录之外）。根目录下的子文件夹**只按结构推定**——**其下存在**符合项目目录形式规则的子文件夹才算类型目录（**名字完全不参与判定**），否则完全不扫。另有 **`checkHierarchy()`**（纯函数）强制层级：一个根目录、根目录之下只能是类型目录、类型目录之下只能是项目目录、根目录之内的另行指定必须正好在它该在的那一层；保存时校验不过就 400，组合层 / 手工配置则忽略该项并把原因记下来。
- **`settings.ts`** —— 用户设置（`$DSH_HOME/settings.yaml` 的 `dslegal:` 段）：三级目录 + 四象限颜色，含旧键 `dataRoot` 的兼容读取。
