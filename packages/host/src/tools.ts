...
const PROJECT = {
  type: 'string',
  required: true,
  description: '项目名（项目文件夹名）。若同名项目存在于多个类型目录，请同时提供 typeDir。',
} as const

const TYPE_DIR = {
  type: 'string',
  description: '类型目录名（如「诉讼案件」），用于消歧同名项目。',
} as const
...
export function registerTools(ctx: Context, deps: ToolDeps): void {
  ctx.tools.register(
    defineTool({
      ...
      description:
        '列出已就绪的法律服务项目（项目目录里含「0. 协作/1. 工作日志.md」）。项目来自根目录下的各类型目录，以及另行指定的类型目录 / 项目目录（可在根目录之外）。返回项目名、类型目录、服务类别与工作日志路径。读写日程/待办前先用它确认项目。',
      parameters: { typeDir: TYPE_DIR },
      output: PROJECT_LIST_OUTPUT,
      async execute(args) {
        const { projects: all, incomplete } = await deps.scan()
        const projects =
          args.typeDir === undefined
            ? all
            : all.filter((item) => item.typeDir === args.typeDir)
        const lines = projects.map(
          (item) =>
            `- ${item.typeDir} / ${item.project}：类别 ${item.category ?? '未知'}${
        ...
        return {
          ...
          message:
            projects.length === 0
              ? `没有找到已就绪的项目（需要「<项目目录>/0. 协作/1. 工作日志.md」）。${incompleteNote}`
  ...
  ctx.tools.register(
    defineTool({
      ...
      output: LIST_OUTPUT,
      async execute(args) {
        const location = await requireProject(deps, args.project, args.typeDir)
  ...
  ctx.tools.register(
    defineTool({
      ...
      parameters: { project: PROJECT, typeDir: TYPE_DIR, target: TARGET },
      output: WRITE_OUTPUT,
      async execute(args) {
        const location = await requireProject(deps, args.project, args.typeDir)
        const snapshot = await deps.readWorkLog(location.workLogPath)
  ...
  ctx.tools.register(
    defineTool({
      ...
      description:
        '跨项目汇总查询：统计各项目的待办与日程（含进行中数量），可限定类型目录、单个项目与日期范围。用于回答「今天/本周有什么安排」「哪些项目有未完成待办」。',
      parameters: {
        typeDir: TYPE_DIR,
      ...
      async execute(args) {
        ...
        const targets = all.filter(
          (item) =>
            (args.typeDir === undefined || item.typeDir === args.typeDir) &&
        ...
        for (const target of targets) {
          ...
          rows.push(row)
          lines.push(
            `- ${target.typeDir} / ${target.project}：待办 ${summary.todoPending} 未完成 / ${summary.todoDone} 已完成；日程 ${summary.scheduleTotal} 条，进行中 ${summary.ongoing}`,
          )
        }
