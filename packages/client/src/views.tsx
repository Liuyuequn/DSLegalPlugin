...
const HELP_SECTIONS: readonly {
  ...
  },
  {
    ...
    more: [
      '这条路径靠三个名字定位，**三级目录都可以在顶部「插件设置」里自己设**：第一级的「根目录」是放各类型目录的那一层；第二级的「类型目录」默认认根目录下的子文件夹——**名字属于默认那几类**（诉讼案件 / 法律顾问 / 知产代理 / 专项服务 / 法律咨询）的照认，**名字不在其中的，只要它下面确实有符合项目目录形式规则的项目文件夹，也会被自动认出来**（认出来之后不限类别）；两条都不满足的文件夹不会被扫，所以根目录里放归档、草稿，乃至**案件文件夹本身**，都不会被误当成类型目录。类型目录也可以另行指定成根目录之外的目录（挂载点、另一个盘都行）；第三级的「项目目录」（案例文件夹）默认是类型目录下的子文件夹，同样可以另行指定，直接把某个案件文件夹指过来也是合法的。',
    ...
    <div style={{ padding: S.xl, display: 'flex', flexDirection: 'column', gap: S.xxl, maxWidth: 640 }}>
      <section style={{ display: 'flex', flexDirection: 'column', gap: S.md }}>
        <div style={UI.sectionHint}>
          ...
          下面两块「另行指定」就是给根目录之外的目录用的。根目录下名称不在默认映射里的
          文件夹，只要它下面含符合这条路径的项目文件夹，也会被自动认作类型目录。
