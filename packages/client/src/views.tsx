...
const HELP_SECTIONS: readonly {
  ...
  },
  {
    ...
    more: [
      '这条路径靠三个名字定位，**三级目录都可以在顶部「插件设置」里自己设**：第一级的「根目录」是放各类型目录的那一层；第二级的「类型目录」**完全由结构推定**——根目录下哪个子文件夹里有符合这条路径的项目文件夹，它就是类型目录，**叫什么名字都行**（名字不参与判定）；里面没有合格项目的文件夹不会被扫，所以根目录里放归档、草稿，乃至**案件文件夹本身**，都不会被误当成类型目录。类型目录也可以另行指定成根目录之外的目录（挂载点、另一个盘都行）；第三级的「项目目录」（案例文件夹）默认是类型目录下的子文件夹，同样可以另行指定，直接把某个案件文件夹指过来也是合法的。',
      '**层级是硬的，不是建议**：一个根目录之下只能有类型目录（案件文件夹不能直接放在根目录下、也不能把根目录下的文件夹指定成项目目录）；类型目录之下只能有项目目录，类型目录之间不能互相嵌套；整个插件只认一个根目录。**根目录之外**可以另行指定**独立存在**的类型目录或项目目录，那里不受这三层约束。位置不合法的「另行指定」在保存时会被**直接拒绝**并写明原因。',
      ...
      'H1 标题即服务类别，例如「# 工作日志_民事」；**目录名不表示类别**——类别只看 H1。缺了或认不出来只是类别显示"未知"，功能不受影响。',
      '待办条目：- [ ] [标题]，[优先级]，[备注]',
...
export function SetupForm(props: {
  ...
  readonly error: string | null
  /**
   * 违反目录层级、**已被 host 忽略**的项（组合层配置 / 手工改过的 `settings.yaml` 才会有）。
   * 保存时的违规走 `error`，这一份说的是"当前生效的配置里哪一条被丢掉了"。
   */
    ...
    <div style={{ padding: S.xl, display: 'flex', flexDirection: 'column', gap: S.xxl, maxWidth: 640 }}>
      <section style={{ display: 'flex', flexDirection: 'column', gap: S.md }}>
        <div style={UI.sectionHint}>
          ...
          下面两块「另行指定」就是给根目录之外的目录用的。根目录下哪个子文件夹算类型目录
          只看结构：里面有符合这条路径的项目文件夹就是，名字随便叫什么。
        ...
        <div style={UI.sectionHint}>
          层级规则：根目录之下只能有类型目录（不能把案件文件夹直接指定成项目目录）；类型目录
          之下只能有项目目录，类型目录之间不能互相嵌套。根目录之外可以另行指定独立存在的
          类型目录或项目目录。
        </div>
        {props.hierarchyIssues === undefined || props.hierarchyIssues.length === 0 ? null : (
          <div style={{ ...FONT.secondary, color: T.error }} role="alert">
            <div>以下设置违反目录层级，已被忽略（改好后再保存一次即可生效）：</div>
            {props.hierarchyIssues.map((line) => (
